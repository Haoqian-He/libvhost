...
static void vhost_vq_init_packed(struct libvhost_virt_queue* vq, struct libvhost_ctrl *ctrl) {
    ...
    vq->packed_ring = true;
    // TODO: support indirect buffer.

    ring_size_in_bytes = vq->size * sizeof(struct vring_packed_desc);
    q_mem = libvhost_malloc(ctrl, ring_size_in_bytes);
    CHECK(q_mem);
    memset(q_mem, 0, ring_size_in_bytes);
    vq->packed.vring.desc = (struct vring_packed_desc*)q_mem;
    ...
    event_size_in_bytes = vq->size * sizeof(struct vring_packed_desc_event);
    q_mem = libvhost_malloc(ctrl, event_size_in_bytes);
    CHECK(q_mem);
    memset(q_mem, 0, event_size_in_bytes);
    vq->packed.vring.driver = (struct vring_packed_desc_event *)q_mem;
    vq->packed.vring.driver->flags = VRING_PACKED_EVENT_FLAG_DISABLE;

    q_mem = libvhost_malloc(ctrl, event_size_in_bytes);
    CHECK(q_mem);
    memset(q_mem, 0, event_size_in_bytes);
    vq->packed.vring.device = (struct vring_packed_desc_event *)q_mem;
...
void virtring_add_packed(struct libvhost_virt_queue* vq, struct iovec* iovec, int num_out, int num_in, void* data) {
    ...
    for (i = 0; i < descs_used; ++i) {
        flags = vq->packed.avail_used_flags |
            ((++c == descs_used) ? 0 : VRING_DESC_F_NEXT) |
        ...
        INFO("ADD new desc(%d) addr(%llx) len(%d) id(%d) flags(%x %s)\n", n,
            (vq->packed.vring.desc[n].addr), (vq->packed.vring.desc[n].len),
            (vq->packed.vring.desc[n].id), (flags), flags_name);
        if (++n >= vq->size) {
            n = 0;
...
static inline bool is_used_desc_packed(const struct libvhost_virt_queue *vq,
                                       uint16_t idx, bool used_wrap_counter) {
    bool avail, used;
    uint16_t flags;
...
int virtring_get_packed(struct libvhost_virt_queue* vq, VhostEvent* event) {
    ...
    DEBUG("[VIRTIO] last_used_idx: %d, used_wrap_counter: %d, free_head: %d"
...
int virtqueue_get(struct libvhost_virt_queue* vq, VhostEvent* event) {
    return vq->packed_ring ? virtring_get_packed(vq, event) :
                virtring_get_split(vq, event);
}
